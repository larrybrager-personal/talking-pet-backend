...
class JobsEndpointResponseContractTestCase(unittest.TestCase):
    ...
    def test_jobs_prompt_only_accepts_alias_payload_fields(self):
        with (
            patch("main._resolve_job_model", new_callable=AsyncMock) as mock_resolve,
            patch(
                "main.generate_video_from_prompt", new_callable=AsyncMock
            ...
            response = self.client.post(
                ...
                json={
                    ...
                    "prompt": "hello",
                    "modelOverride": "bytedance/seedance-1-pro-fast",
                    "modelParams": {"fps": 24},
                    "userContext": {
                        "id": "00000000-0000-0000-0000-000000000000",
                        "planTier": "creator",
    ...
    def test_jobs_prompt_tts_accepts_alias_payload_fields(self):
        with (
            patch("main._resolve_job_model", new_callable=AsyncMock) as mock_resolve,
            patch("main.elevenlabs_tts_bytes", new_callable=AsyncMock) as mock_tts,
            patch(
    ...
    def test_jobs_prompt_only_persists_final_url_as_canonical_video_reference(self):
        with (
            patch("main._resolve_job_model", new_callable=AsyncMock) as mock_resolve,
            patch(
                "main.generate_video_from_prompt", new_callable=AsyncMock
        ...
        self.assertEqual(insert_kwargs["final_url"], "https://supabase/final.mp4")
        self.assertEqual(
            insert_kwargs["provider_video_url"], "https://provider/video.mp4"
        )
